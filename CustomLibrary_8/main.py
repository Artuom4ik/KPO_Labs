"""
v.1.0.8
main_update: пользовательские библиотеки
author: [
    'Herasimau Artsem',
    'Shaplavskiy Mikita'
]
...
# Dynamic import of calculator_lib (static library)
try:
    from calculator_lib import CalculatorModel
    print("Static library 'calculator_lib' loaded successfully")
except ImportError:
    print("Static library 'calculator_lib' not found, trying to load from local path")
    try:
        # Add the calculator_lib directory to the Python path
        sys.path.append(os.path.abspath("calculator_lib"))
        from calculator_lib import CalculatorModel
        print("Static library 'calculator_lib' loaded from local path")
    except ImportError:
        print("Failed to load static library 'calculator_lib'")
        sys.exit(1)

# Dynamic import of developer_lib (dynamic library with static linking)
try:
    from developer_lib import DeveloperInfo
    print("Dynamic library 'developer_lib' loaded successfully (static linking)")
    developer_info = DeveloperInfo()
except ImportError:
    print("Dynamic library 'developer_lib' not found, trying to load dynamically")
    try:
        # Dynamic loading of the developer_lib
        ...
        if spec is not None:
            ...
            developer_info = DeveloperInfo()
            print("Dynamic library 'developer_lib' loaded successfully (dynamic linking)")
        else:
            # Try to load from local path
            developer_lib_path = os.path.abspath("developer_lib/developer_info.py")
            if os.path.exists(developer_lib_path):
                spec = importlib.util.spec_from_file_location("developer_info", developer_lib_path)
                module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(module)
                DeveloperInfo = module.DeveloperInfo
                developer_info = DeveloperInfo()
                print("Dynamic library 'developer_lib' loaded from local path")
            else:
                print("Failed to load dynamic library 'developer_lib'")
                # Create a minimal DeveloperInfo class as fallback
                class DeveloperInfo:
                    @staticmethod
                    def get_about_text():
                        return "Developer information not available"
        ...
        print(f"Error loading dynamic library 'developer_lib': {str(e)}")
        # Create a minimal DeveloperInfo class as fallback
        class DeveloperInfo:
            @staticmethod
            def get_about_text():
...
class Calculator(ctk.CTk):
    ...
    def create_menu(self):
        ...
        resources_menu.add_command(label="О разработчиках", command=self.show_about)
    ...
    def show_about(self):
        try:
            # Get information from the developer_lib
            about_text = DeveloperInfo.get_about_text()
            ...
            label = ctk.CTkLabel(about_window, text=about_text, wraplength=380)
            label.pack(padx=10, pady=10, expand=True)
        except Exception as e:
            messagebox.showerror("Error", f"Could not show about information: {str(e)}")
